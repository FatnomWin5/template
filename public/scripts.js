const API_KEY = 'ffabe702da033934777f5163ba7e8db3';
const MAIN_URL = `https://ws.audioscrobbler.com/2.0/?method=tag.gettopalbums&tag=edm&api_key=ffabe702da033934777f5163ba7e8db3&format=json`;
const ERROR = "Данные по запросу не найдены"
...
function getTop() 
{
	getData(MAIN_URL).then((data) => {
		if (typeof data === 'undefined')
			...
			data.albums.album.forEach((item) => {createTopArtistsCard(item);})
	});
	return;
}
...
document.querySelector(".header__search").addEventListener("keydown", (e) => {
	...
	if (e.keyCode === 13) {
		if (e.target.value === "") {
			...
			getData(
					`https://ws.audioscrobbler.com/2.0/?method=track.search&limit=100&track=${e.target.value}&api_key=${API_KEY}&format=json`
				)
				.then((result) => {
					content.textContent = "";
...
function createSearchValues(item) {
	...
	artistData.appendChild(createContainer(item));
}
...
function createInfoTrackContainer(item) {
	...
	dataContainer.className = "music p";

	dataContainer.appendChild(createMusicionInfo(item));
...
function createMusicionInfo(item) {
	const dataAlbum = document.createElement('div');
...
function createImage() {
	...
	buttonData.className = "music img";
